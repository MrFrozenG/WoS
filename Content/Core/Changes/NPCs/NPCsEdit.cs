...
namespace WoS.Content.Core.Changes.NPCs;
...
public class NPCsEdit : GlobalNPC
{
    ...
    {
        if (ModContent.GetInstance<MainConfig>().TownNPCsInvincible)
        {
            if (npc.townNPC)
            {
    ...
    {
        if (ModContent.GetInstance<MainConfig>().TownNPCsSpecialShop || ModContent.GetInstance<MainConfig>().TownNPCsInvincible)
        {
